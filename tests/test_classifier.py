...
class TestOptimizedClassifier:
    ...
    def test_fast_classify_relationships(self, classifier):
        ...
        result = classifier.fast_classify("My colleague Sarah helped with the code review")
        
        assert "colleague" in result.primary_path or "relationships" in result.primary_path
        assert result.confidence > 0.5
    ...
    def test_batch_classify(self, classifier):
        ...
        assert "work" in results[1].primary_path or "professional" in results[1].primary_path
        assert "prefer" in results[2].primary_path or "preferences" in results[2].primary_path
...
class TestClassificationAccuracy:
    ...
    def test_identity_classification(self, classifier):
        ...
        for text, expected_keyword in test_cases:
            ...
            assert expected_keyword in result.primary_path.lower() or \
                   expected_keyword in result.reasoning.lower(), \
                   f"Failed for: {text}"
    ...
    def test_professional_classification(self, classifier):
        ...
        for text, expected_keyword in test_cases:
            ...
            assert "professional" in path_lower or \
                   "work" in path_lower or \
                   expected_keyword in path_lower, \
