...
class HierarchicalSearchEngine:
    ...
    def _build_search_index(self):
        ...
        self.query_patterns = {
            ...
            "what.*working on": ["experience.projects.current", "context.current.session"],
            "what.*goal": ["goals"],
            ...
            "personal": ["profile.personal", "preferences.personal", "goals.categories.personal"],
    ...
    async def search(self,
                    ...
                    context: Optional[Dict] = None) -> List[SearchResult]:
        """
        ...
        logger.info(f"Search completed in {search_time:.2f}ms, found {len(final_results)} results")
        
        return final_results
    
    async def _map_query_to_paths(self, query: str, context: Optional[Dict]) -> List[str]:
        ...
        for pattern, suggested_paths in self.query_patterns.items():
            if pattern in query_lower or (pattern.startswith("what") and pattern[4:] in query_lower):
    ...
    async def _search_specific_to_general(self, 
                                         namespace: str,
                                         search_paths: List[str]) -> List[SearchResult]:
        """
            ...
            for level, path_prefix in enumerate(hierarchy):
                ...
                for key, content in items:
                    result = SearchResult(
                        key=key,
                        content=content,
                        relevance_score=1.0 / (level + 1),  # Higher score for more specific
    ...
    async def _search_general_to_specific(self,
                                         namespace: str,
                                         search_paths: List[str]) -> List[SearchResult]:
        """
    ...
    async def _search_breadth_first(self,
                                   namespace: str,
                                   search_paths: List[str]) -> List[SearchResult]:
        """
            ...
            for path in depth_groups[depth]:
                ...
                for key, content in items:
                    result = SearchResult(
                        key=key,
                        content=content,
                        relevance_score=1.0 / (abs(depth - 3) + 1),  # Prefer medium depth
                        semantic_distance=0,
                        confidence=1.0,
                        timestamp=time.time(),
                        namespace=namespace
    ...
    async def _search_best_match(self,
                                namespace: str,
                                search_paths: List[str]) -> List[SearchResult]:
        """
    ...
    async def multi_hop_search(self,
                              queries: List[str],
                              namespace: str,
                              combine_strategy: str = "union") -> List[SearchResult]:
        """
        ...
        if combine_strategy == "intersection":
            ...
            return sorted(combined.values(), key=lambda r: r.relevance_score, reverse=True)