...
class ProllyTreeMemoryStoreManager(MemoryStoreManager):
    ...
    async def search_memories(self,
        ...
        logger.info(f"Search completed in {search_time:.2f}ms, found {len(memories)} memories")
    ...
    async def get_memory_versions(self,
                                 semantic_key: str,
                                 namespace: str,
    ...
    async def time_travel(self,
                        namespace: str,
                        target_time: Union[datetime, float]) -> Dict[str, Any]:
        """
    ...
    async def compare_memory_states(self,
                                   namespace: str,
                                   time1: Union[datetime, float],
                                   time2: Union[datetime, float]) -> Dict[str, Any]:
        """
    ...
    async def branch_memories(self,
                            namespace: str,
                            branch_name: str) -> str:
        """
    ...
    async def merge_memories(self,
                           ...
                           strategy: str = "ours") -> Dict[str, Any]:
        """
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        if metrics["search_time_ms"]:
            metrics["avg_search_time_ms"] = sum(metrics["search_time_ms"]) / len(metrics["search_time_ms"])
            metrics["p95_search_time_ms"] = sorted(metrics["search_time_ms"])[int(len(metrics["search_time_ms"]) * 0.95)] if len(metrics["search_time_ms"]) > 1 else metrics["search_time_ms"][0]
        
        if metrics["write_time_ms"]:
            metrics["avg_write_time_ms"] = sum(metrics["write_time_ms"]) / len(metrics["write_time_ms"])
        
        if metrics["classification_time_ms"]:
            metrics["avg_classification_time_ms"] = sum(metrics["classification_time_ms"]) / len(metrics["classification_time_ms"])
