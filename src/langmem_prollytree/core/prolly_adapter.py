...
class ProllyTreeStore(BaseStore):
    ...
    async def asearch(self, namespace: str, prefix: str) -> List[Tuple[str, Any]]:
        ...
        try:
            ...
            items = await self.tree.range_query_async(
                search_prefix.encode(),
                (search_prefix + "\xff").encode()
            )
    ...
    async def get_history(self, namespace: str, key: str, limit: int = 10) -> List[MemoryItem]:
        """
    ...
    def import_namespace(self, input_path: str, namespace: Optional[str] = None) -> None:
        """
