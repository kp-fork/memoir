...
class BenchmarkSuite:
    ...
    def __init__(self, num_memories: int = 100, num_searches: int = 50):
        ...
        self.replacements = {
            ...
            "project_type": ["machine learning", "web application", "mobile app", "blockchain"],
    ...
    async def benchmark_storage_performance(self, memory_manager: ProllyTreeMemoryStoreManager) -> Dict[str, Any]:
        ...
        return {
            ...
            "p95_storage_time_ms": sorted(storage_times)[int(len(storage_times) * 0.95)],
            "storage_times": storage_times
        }
    
    async def benchmark_search_performance(self, memory_manager: ProllyTreeMemoryStoreManager, user_id: str) -> Dict[str, Any]:
        ...
        for strategy in strategies:
            ...
            results[strategy.value] = {
                ...
                "p95_search_time_ms": sorted(strategy_times)[int(len(strategy_times) * 0.95)],
    ...
    async def benchmark_classification_performance(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "p95_fast_classify_time_ms": sorted(fast_times)[int(len(fast_times) * 0.95)],
    ...
    async def run_full_benchmark(self) -> Dict[str, Any]:
        ...
        search_results = await self.benchmark_search_performance(memory_manager, user_id)
    ...
    def print_results(self, results: Dict[str, Any]):
        ...
        print(f"  • Min/Max: {storage['min_storage_time_ms']:.2f}ms / {storage['max_storage_time_ms']:.2f}ms")
        ...
        print(f"-" * 65)
        print(f"Memory Storage     | 200-600ms       | {avg_storage:>8.1f}ms   | {300/avg_storage:>6.0f}x faster")
        print(f"Memory Search      | 150-750ms       | {avg_search:>8.1f}ms   | {400/avg_search:>6.0f}x faster")
        print(f"Classification     | 2000-5000ms     | {avg_classify:>8.1f}ms   | {3000/avg_classify:>6.0f}x faster")
        
        total_vanilla = 3500  # Conservative estimate for vanilla LangMem
        total_prolly = avg_storage + avg_search + avg_classify
        
        print(f"Total per memory   | ~3500ms         | {total_prolly:>8.1f}ms   | {total_vanilla/total_prolly:>6.0f}x faster")
