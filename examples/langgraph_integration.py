...
"""
        ...
        await self._store_conversation_memories(message, response["messages"][-1].content)
    ...
    async def _store_conversation_memories(self, user_message: str, assistant_response: str):
    ...
    async def _extract_insights(self, user_message: str, assistant_response: str) -> List[str]:
    ...
    async def get_user_profile(self) -> Dict[str, Any]:
        ...
        for query, expected_category in profile_queries:
            ...
            profile[expected_category] = [
                {"content": m.content, "relevance": m.metadata.get("relevance_score", 0)}
                for m in memories
            ]
    ...
    async def get_memory_statistics(self) -> Dict[str, Any]:
        ...
        if self.user_id:
            ...
            optimization = await self.memory_manager.optimize_memory_layout(self.user_id)
...
async def demo_enhanced_agent():
    ...
    print(f"Average conversation turn: {total_processing_time / len(conversation_turns):.2f}ms")
