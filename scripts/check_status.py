...
class StatusChecker:
    ...
    def _check_python_version(self) -> Dict[str, Any]:
        ...
        if success:
            ...
            return {
                "success": compatible,
                "version": version,
                "message": f"Python {version}" + ("" if compatible else " (requires >= 3.9)")
            }
    ...
    def _check_pip_install(self) -> Dict[str, Any]:
        ...
        return {
            "success": success,
            "message": "Package installation successful" if success else f"Installation failed: {stderr}"
        }
    ...
    def _check_dev_dependencies(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tools": results,
            "message": "All dev tools available" if all_available else f"Missing tools: {[k for k, v in results.items() if not v]}"
        }
    ...
    def _check_formatting(self) -> Dict[str, Any]:
        ...
        isort_success, isort_out, isort_err = self.run_command([
            sys.executable, "-m", "isort", "--check-only", "src/", "tests/", "examples/"
        ])
        ...
        return {
            ...
            "black": black_success,
            "isort": isort_success,
            "message": "Code formatting OK" if success else "Code formatting issues found"
        }
    ...
    def _check_linting(self) -> Dict[str, Any]:
        ...
        return {
            "success": success,
            "message": "No linting issues" if success else f"Linting issues found: {stderr}"
        }
    
    def _check_type_checking(self) -> Dict[str, Any]:
        """Check type annotations with MyPy."""
        success, stdout, stderr = self.run_command([
            sys.executable, "-m", "mypy", "src/langmem_prollytree", "--ignore-missing-imports"
        ])
        
        return {
            "success": success,
            "message": "Type checking passed" if success else f"Type errors found: {stderr}"
        }
    ...
    def _check_security(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "message": "Security checks passed" if bandit_success else f"Security issues found: {bandit_err}"
        }
    ...
    def check_tests(self) -> Dict[str, Any]:
        ...
        success, stdout, stderr = self.run_command([
            ...
            "--cov=langmem_prollytree", "--cov-report=term-missing"
        ])
    ...
    def check_examples(self) -> Dict[str, Any]:
        ...
        return {
            "status": all_success,
            "examples": results,
            "message": "All examples run successfully" if all_success else "Some examples failed"
        }
    ...
    def check_performance(self) -> Dict[str, Any]:
        ...
        success, stdout, stderr = self.run_command([
            sys.executable, "examples/performance_benchmark.py"
        ], timeout=120)
            ...
            try:
                ...
                for line in lines:
                    if "Average search time:" in line:
                        metrics["search_time_ms"] = float(line.split(':')[1].strip().rstrip('ms'))
                    elif "Average storage time:" in line:
                        metrics["storage_time_ms"] = float(line.split(':')[1].strip().rstrip('ms'))
                    elif "Average classification time:" in line:
                        metrics["classification_time_ms"] = float(line.split(':')[1].strip().rstrip('ms'))
            except:
                pass
        
        # Check if performance targets are met
        performance_ok = (
            metrics.get("search_time_ms", 999) < 10 and
            metrics.get("storage_time_ms", 999) < 50 and
            metrics.get("classification_time_ms", 999) < 10
        )
        ...
        return {
            ...
            "message": "Performance benchmarks completed" if success else f"Benchmark failed: {stderr}"
        }
    ...
    def print_summary(self, report: Dict[str, Any]):
        ...
        if perf["success"] and perf.get("metrics"):
            ...
            print(f"  📊 Classification: {metrics.get('classification_time_ms', 'N/A')}ms")
